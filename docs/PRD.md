...
- The customer page has one large personal-order card with Aura progress, a prominent count of ordered items ahead, and connection/freshness information. Ordered work uses the standard location-colored Aura, while the ready-to-pick-up state uses daisyUI's contained dual Aura. It does not render ready or ordered lists.
